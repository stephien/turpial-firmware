...
public:
    ...
    ws_client_t wsConnectClient(struct netconn* conn,
        char* url,
    ...
    void wsDisconnectClient(ws_client_t* client);
    bool wsIsConnected(ws_client_t client);                                                          // returns 1 if connected, status updates after send/read/connect/disconnect
    void wsSend(ws_client_t* client, WEBSOCKET_OPCODES_t opcode, char* msg, uint64_t len, bool mask); // sends message. this function performs the masking
    char* wsRead(ws_client_t* client, ws_header_t* header);                                           // unmasks and returns message. populates header.
