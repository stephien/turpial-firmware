...
ws_client_t WebSocket::wsConnectClient(struct netconn* conn,
    char* url,
    ...
    void (*scallback)(uint8_t num, WEBSOCKET_TYPE_t type, char* msg, uint64_t len))
{
    ...
    client.url = url;
    client.ping = 0;
    client.last_opcode = static_cast<WEBSOCKET_OPCODES_t>(0);
    client.contin = NULL;
    client.len = 0;
...
void WebSocket::wsDisconnectClient(ws_client_t* client)
{
    //wsSend(client,WEBSOCKET_OPCODE_CLOSE,NULL,0,1); // tell the client to close
    if (client->conn) {
...
bool WebSocket::wsIsConnected(ws_client_t client)
{
...
void WebSocket::wsGenerateMask(ws_header_t* header)
{
    header->param.bit.MASK = 1;
    header->key.full = esp_random(); // generate a random 32 bit number
}

void WebSocket::wsEncryptDecrypt(char* msg, ws_header_t header)
{
...
void WebSocket::wsSend(ws_client_t* client, WEBSOCKET_OPCODES_t opcode, char* msg, uint64_t len, bool mask)
{
    char* out;
    char* encrypt;
    uint64_t pos;
    ...
    if (mask) {
        wsGenerateMask(&header);      // get a key
        encrypt = (char*)malloc(len); // allocate memory for the encryption
        memcpy(encrypt, msg, len);
        wsEncryptDecrypt(encrypt, header); // encrypt it!
        pos += 4;                          // add the position
    }

    true_len = pos + len; // get the length of the entire message
    pos = 2;
    out = (char*)malloc(true_len); // allocate dat memory
...
char* WebSocket::wsRead(ws_client_t* client, ws_header_t* header)
{
    char* ret;
    char* append;
    err_t err;
    ...
    ret = (char*)malloc(header->length + 1); // allocate memory, plus a bit
    if (!ret) {
        netbuf_delete(inbuf);
        free(buf);
        return NULL;
    ...
    wsEncryptDecrypt(ret, *header);             // unencrypt, if necessary
    ...
    if (header->param.bit.FIN == 0) { // if the message isn't done
        if ((header->param.bit.OPCODE == WEBSOCKET_OPCODE_CONT) &&
            ...
            client->contin = (char*)malloc(header->length);
            memcpy(client->contin, ret, header->length);
            client->len = header->length;
            client->last_opcode = static_cast<WEBSOCKET_OPCODES_t>(header->param.bit.OPCODE);
...
char* WebSocket::wsHashHandshake(char* handshake, uint8_t len)
{
